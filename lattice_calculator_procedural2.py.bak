...
def CleanArgs(**args):
    ...
    For shorter arrays, it pads them with the last value in the array.  It returns a dictionary based on the calling args"""
     ...
     for name,val in args.items():
          ...
          elif type(val)==type([1,2,3]):
               npts.append(len(val))
          else:
               #print(name)
               #print(args[name])
               #print(args[name].shape)
...
class Lattice(object):
     ...
     def setvals(self):
          ...
          x,y,z=StandardSystem(self._orient1,self._orient2,self)
          self.x=x
          self.y=y
          self.z=z
...
def SpecWhere(M2,S1,S2,A2,EXP,lattice,orientation,instrument):
   ...
   and final neutron energies.  Angles are given in radians"""
     ...
     print(neworientation.orient1, neworientation.orient2)
     ...
     return H/2/pi,K/2/pi,L/2/pi,E,Q,Ei,Ef
